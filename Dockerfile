...
ENV NGINX_LUA_HTTP_MODULE_VERSION 0.10.15
ENV NGINX_LUA_STREAM_MODULE_VERSION 0.0.3
ENV NGINX_LUA_RESTY_CORE_VERSION 0.1.17
...
RUN set -x \
    && apt-get update \
    && apt-get install -y wget build-essential libpcre3 libpcre3-dev libghc-zlib-dev libssl-dev
RUN set -x \
...
COPY --from=builder /usr/local/lib/lua/5.1 /usr/local/lua
